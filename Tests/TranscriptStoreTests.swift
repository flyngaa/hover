...
@Suite final class TranscriptStoreTests {
    ...
    @Test func writeAndReadRoundTrip() throws {
        ...
        store.write(title: "t", body: "hello", to: url)

        let raw = try String(contentsOf: url, encoding: .utf8)
        #expect(raw == "# t\n\nhello\n")
        // content() strips the heading; the body keeps its trailing newline.
        #expect(store.content(of: transcript(url, name: "t")) == "hello\n")
    }

    @Test func loadListsTranscriptsAndGroupsAndSkipsReserved() throws {
        // A transcript at the root.
        store.write(title: "root-note", body: "a", to: dir.appendingPathComponent("root-note.md"))
        ...
        store.write(title: "group-note", body: "b", to: group.appendingPathComponent("group-note.md"))
        // A file inside the reserved "models" folder — must be ignored.
        let models = dir.appendingPathComponent("models")
        try FileManager.default.createDirectory(at: models, withIntermediateDirectories: true)
        store.write(title: "ignore", body: "c", to: models.appendingPathComponent("ignore.md"))

        let library = store.load(in: dir)
        #expect(library.transcripts.count == 2)
        #expect(library.groups == ["GroupA"])
        #expect(library.transcripts.contains { $0.group == "GroupA" })
    }

    @Test func matchesByNameAndContent() {
        let url = dir.appendingPathComponent("meeting-notes.md")
        store.write(title: "meeting-notes", body: "the quick brown fox", to: url)
    ...
    @Test func rename() throws {
        ...
        store.write(title: "old", body: "x", to: url)

        let newURL = try #require(store.rename(transcript(url, name: "old"), to: "new"))
        #expect(newURL.lastPathComponent == "new.md")
    ...
    @Test func moveIntoGroup() throws {
        ...
        store.write(title: "m", body: "x", to: url)

        let newURL = try #require(store.move(transcript(url, name: "m"), toGroup: "G", in: dir))
        #expect(newURL == dir.appendingPathComponent("G/m.md"))
        #expect(FileManager.default.fileExists(atPath: newURL.path))
    }

    @Test func delete() {
        let url = dir.appendingPathComponent("d.md")
        store.write(title: "d", body: "x", to: url)
        #expect(FileManager.default.fileExists(atPath: url.path))

        store.delete(transcript(url, name: "d"))
        #expect(FileManager.default.fileExists(atPath: url.path) == false)
    }
    ...
    @Test func relocateMovesFilesAndKeepsGroups() throws {
        ...
        store.write(title: "root", body: "a", to: rootURL)
        store.write(title: "grouped", body: "b", to: groupURL)

        let moved = store.relocate(
            [transcript(rootURL, name: "root"), transcript(groupURL, name: "grouped", group: "GroupA")],
            to: destination
        )
    ...
    @Test func relocateDoesNotOverwriteExistingFile() throws {
        ...
        store.write(title: "clash", body: "theirs", to: destination.appendingPathComponent("clash.md"))

        let source = dir.appendingPathComponent("clash.md")
        store.write(title: "clash", body: "ours", to: source)

        #expect(store.relocate([transcript(source, name: "clash")], to: destination) == 1)

        let kept = try String(contentsOf: destination.appendingPathComponent("clash.md"), encoding: .utf8)
        let moved = try String(contentsOf: destination.appendingPathComponent("clash 2.md"), encoding: .utf8)
    ...
    @Test func relocateToSameFolderDoesNothing() {
        let url = dir.appendingPathComponent("stay.md")
        store.write(title: "stay", body: "x", to: url)

        #expect(store.relocate([transcript(url, name: "stay")], to: dir) == 0)
        #expect(FileManager.default.fileExists(atPath: url.path))
    }
}

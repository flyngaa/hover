...
@Suite struct CLISetupTests {
    ...
    @Test func successfulAndFailedFetchesHaveStableProcessContracts() {
        #expect(HoverCLI.setupCompletion(isComplete: true, errorDescription: nil) == .init(
            exitCode: 0,
            standardError: "Model data is ready.\n"
        ))
        #expect(HoverCLI.setupCompletion(isComplete: false, errorDescription: "Network dropped.") == .init(
            exitCode: 1,

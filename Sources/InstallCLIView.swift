...
final class InstallCLIController {
    ...
    func refresh() {
        status = installCLI?.status() ?? .needsRepair
    }
    ...
    func install() async {
        ...
        do {
            ...
            status = installCLI.status()
        } catch {
            errorMessage = "Install CLI needs administrator approval. Try again."
        }
        isInstalling = false
    }
}
...
struct InstallCLIView: View {
    ...
    private var statusMessage: String {
        ...
        case .foreign:
            return "A different hover command is installed. Hover can replace it if you choose Install."
        }
    }
}

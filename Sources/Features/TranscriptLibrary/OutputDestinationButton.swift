...
struct OutputDestinationButton: View {
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 7) {
                destinationIcon(library.currentOutputDestination)
                    .foregroundStyle(.white)
                Text(library.currentOutputDestination.name)
        ...
        .buttonStyle(.plain)
        .help("Where transcripts are saved: \(library.outputDirectoryLabel)")
        ...
        .confirmationDialog(
            moveTitle,
            isPresented: .init(
                get: { library.pendingOutputChange != nil },
                set: { if !$0 { library.cancelPendingOutputChange() } }
            ),
            titleVisibility: .visible,
            presenting: library.pendingOutputChange
        ) { pending in
            Button("Move \(pending.transcriptCount == 1 ? "Transcript" : "Transcripts")") {
                appModel.resolvePendingOutputChange(movingTranscripts: true)
            }
            Button("Leave Them") {
                appModel.resolvePendingOutputChange(movingTranscripts: false)
            }
            Button("Cancel", role: .cancel) {
                library.cancelPendingOutputChange()
            }
        } message: { pending in
    ...
    private var destinationList: some View {
        let destinations = library.outputDestinations
        let current = library.currentOutputDestination
        ...
        return VStack(alignment: .leading, spacing: 1) {
            ...
            DestinationRow(
                title: "Choose Folder…",
                isEnabled: recording.canConfigureRecording,
                select: chooseFolder
            )
        }
        .padding(.bottom, 7)
    ...
    private func row(_ destination: OutputDestination, isCurrent: Bool) -> DestinationRow {
        DestinationRow(
            ...
            reveal: { reveal(destination.directory) }
        )
    }
    ...
    private func select(_ destination: OutputDestination) {
        ...
        DispatchQueue.main.async {
            appModel.requestOutputChange(to: destination)
        }
    }
    ...
    private func chooseFolder() {
        ...
        DispatchQueue.main.async {
            ...
            appModel.requestOutputChange(toFolder: url)
        }
    }
    ...
    private var moveTitle: String {
        guard let pending = library.pendingOutputChange else { return "" }
        let noun = pending.transcriptCount == 1 ? "transcript" : "transcripts"
        return "Move your \(noun) to \(pending.destination.name)?"
    }

    private func moveMessage(_ pending: PendingOutputDestinationChange) -> String {
        let count = pending.transcriptCount
        let noun = count == 1 ? "transcript" : "transcripts"
        return """
            You have \(count) \(noun) in \(TranscriptLibraryModel.shortPath(pending.origin)).

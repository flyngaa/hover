...
struct PermissionRequestSheet: View {
    ...
    var body: some View {
        if let request = engine.permissionRequest {
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(spacing: 10) {
                    Button("Cancel") { engine.dismissPermissionRequest() }
                    ...
                    if let fallbackButton = request.fallbackButton {
                        Button(fallbackButton) {
                            Task { await engine.recordWithReducedInput() }
                        }
                    }

                    Button(request.primaryButton) { engine.grantPermission() }

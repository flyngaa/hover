...
@Suite struct ParseNativeSpeakerTurnsTests {
    ...
    @Test func parsesANormalMultiSpeakerResult() {
        let output = """
            ...
            """
        let turns = DiarizationOutputParser.nativeTurns(output) ?? []
    ...
    @Test func emptyOutputYieldsNoTurns() {
        #expect((DiarizationOutputParser.nativeTurns("") ?? []).isEmpty)
        #expect((DiarizationOutputParser.nativeTurns("Started\n") ?? []).isEmpty)
    }
    ...
    @Test func nativeHelperArgumentsCarryTodaysTuning() {
        let args = LocalSpeakerDiarizer.nativeArguments(
